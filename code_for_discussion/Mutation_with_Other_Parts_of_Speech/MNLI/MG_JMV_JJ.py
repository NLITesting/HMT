    ...
    for targetWord in target:
        ...
        for i in range(len(choose_list)):
            ...
            try:
                ...
                if parseMut1.replace(sims[i_list[choose_list[i]]], targetWord) == parse1 and parseMut2.replace(sims[j_list[choose_list[i]]], targetWord) == parse2:
                    passTest = True
                tmpDict = {'targetWord':targetWord,'replaceWord1':sims[i_list[choose_list[i]]],'replaceWord2':sims[j_list[choose_list[i]]],
            ...
            try:
                parseMut1 = nlp.parse(mut1)
                parseMut2 = nlp.parse(mut2)
                if parseMut1.replace(sims[j_list[choose_list[i]]], targetWord) == parse1 and parseMut2.replace(sims[i_list[choose_list[i]]],
                                                                                                 targetWord) == parse2:
                    passTest = True
                tmpDict = {'targetWord': targetWord, 'replaceWord1': sims[j_list[choose_list[i]]], 'replaceWord2': sims[i_list[choose_list[i]]],
                               'mut1': mut1, 'mut2': mut2, 'passTest': passTest}
                mutationDict['mutationList'].append(tmpDict)
            except:
...
def getSynset(targetWord):
    set = wn.synsets(targetWord, pos=wn.ADJ)
    synset = []
    for w in set:
        wset = w.lemma_names()
        for i in wset:
...
def checkSynset(targetWord1,targetWord2):
    set = wn.synsets(targetWord1, pos=wn.ADJ)
    synset = []
    for w in set:
        wset = w.lemma_names()
        for i in wset:
...
print(timeuse)
with open("MG_JMV.json", "w", encoding="utf-8") as f:

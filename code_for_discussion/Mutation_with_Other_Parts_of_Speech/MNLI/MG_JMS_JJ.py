    ...
    for targetWord in target:
        ...
        for i in range(len(choose_list)):
            ...
            try:
                ...
                if parseMut1.replace(sims[i_list[choose_list[i]]], targetWord) == parse1 and parseMut2.replace(sims[i_list[choose_list[i]]], targetWord) == parse2:
                    passTest = True
                tmpDict = {'targetWord':targetWord,'replaceWord1':sims[i_list[choose_list[i]]],'replaceWord2':sims[i_list[choose_list[i]]],
...
print(timeuse)
with open("MG_JMS.json", "w", encoding="utf-8") as f:
